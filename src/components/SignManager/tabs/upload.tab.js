            ...
            reader.onload = async (res) => {
                ...
                if(key == "signature"){
                    ...
                    image.onload = () => {
                        ...
                        ctx.strokeStyle = '#d35400';
                        ctx.stroke();
                        this.setState({initialsBox:initialBoxCanvas.toDataURL()},()=>{
                            this.emitChanges();
                        })
                ...
                <div className={`dropzone ${errors.initials ? 'error' : ''}`}>
                    {
                        initials ? (
                            <Fragment>
                                <i onClick={() => this.removeFile('initials')} className="ni ni-fat-remove remove-icon" />
                                <img src={initials} />
                            </Fragment>
                        ) : (
                            <Fragment>
                                <i className="ni ni-cloud-upload-96 upload-icon" />
                                <button className="btn" onClick={() => this.onClickUpload(this.initialsInput)}>
                                    UPLOAD INITIALS
                                </button>
                                <div className="mt-2 error-message">
                                    {errors.initials}
