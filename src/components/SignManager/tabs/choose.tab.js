...
class ChooseTab extends Component {
    fonts = [
        {
            name: 'HighTide', size: 25
        },
        {
            name: 'MissNeally', size: 25
        },
        {
            name: 'RunWild', size: 25
        },
        {
            name: 'WarmShowers', size: 40
        },
    ];
            ...
            <div className="sign-manager-choose-tab">
                {
                    this.fonts.map((font, i) => (
                        <div key={i} style={{ visibility: 'hidden', height: 0, fontFamily: font.name }}>ABC</div>
                    ))
                }
                {
                    options.length === 0 ? (
                        ...
                        <label key={i} className="sign-box">
                            <input onChange={() => this.onSelect(option)} type="radio" id={`sign-${i}`} name="sign"
                                   value={i} />
                                ...
                                }
                                {
                                    option.initials && initial && (
                                        <div className="sign-image-box sign-image-box-initials">
                                            <span className="signed-by">{initial}</span>
