...
class DrawTab extends Component {
    ...
    onDrawEnd = async () => {
        ...
        onChange({
            signatureBox: await this.generatedBoxVersion('#signature-pad', this.signaturePad),
            signature: this.signaturePad.isEmpty() ? null : this.signaturePad.toDataURL(),
            initials: this.initialsPad.isEmpty() ? null : this.initialsPad.toDataURL(),
            initialsBox: await this.generatedBoxVersion('#initials-pad', this.initialsPad),
        })
    }
                    ...
                    <div className="intro">
                        <label>Draw Signature</label>
                        {
                            this.signaturePad && !this.signaturePad.isEmpty() && (
                                <i onClick={() => this.onClear(this.signaturePad)} className="ni ni-fat-remove" />
                            )
                        }
                    </div>
                </div>
                    ...
                    <div className="intro">
                        <label>Draw Initials</label>
                        {
                            this.initialsPad && !this.initialsPad.isEmpty() && (
                                <i onClick={() =>this.onClear(this.initialsPad)} className="ni ni-fat-remove" />
                            )
                        }
                    </div>
                </div>
