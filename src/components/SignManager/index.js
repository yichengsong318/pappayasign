...
class SignManager extends Component {
    ...
    * */
          ...
          if (userid) {
            ...
            try {
              axios
                ...
                .then((response) => {
                    var Name = response.data.user.UserFirstName + ' ' + response.data.user.UserLastName
                    var Initials = response.data.user.UserFirstName.substr(0, 1).toUpperCase()  + response.data.user.UserLastName.substr(0, 1).toUpperCase()
                    this.setState({
                        name: Name,
                        initial: Initials
                    });
    ...
    render() {
        const {activeTabIndex, name, initial, result, id} = this.state;
                ...
                <div className="modal-content">
                        ...
                        <div className="row">
                            <div className="col-md-6">
                                <FormGroup className="d-flex align-items-center">
                                    <h5 className="mr-3 mb-0">Name</h5>
                    ...
                    <div className="sign-manager-tabs">
                        <Nav className="sign-manager-tabs-selector" tabs>
                            {
                                this.tabs.map((tab, i) => (
                                    <NavItem key={i}>
                                        <NavLink className={activeTabIndex === i ? 'active' : ''} onClick={() => this.onChangeTab(i)}>
                                            {tab.title}
                                        </NavLink>
                                    </NavItem>
                                ))
                        ...
                        <TabContent className="sign-manager-tabs-content">
                            <TabPane>
                                <TabComponent id={id} name={name} initial={initial} onChange={this.onChangeSignCreator}  />
                            </TabPane>
                        </TabContent>
                    </div>
                    <div className="sign-manager-footer">
                        <div>By clicking Create, I agree that the signature and initials will be the electronic representation of my signature and initials for all purposes when I (or my agent) use them on envelopes, including legally binding contracts - just the same as a pen-and-paper signature or initial.</div>
                        <div className="mt-4">
                            <button
                                type="button"
