...
async function setExpiry(Reciever, UserID, DocumentID){
	...
	var today = new Date().toLocaleString().replace(',', '')
		...
		recievers.forEach(function (item, index) {
			...
			recievers[index].RecepientStatus = 'Expiring'
			recievers[index].RecepientDateStatus = today

						client.connect((err) => {
						var queryinside = { UserEmail: item.RecepientEmail};
						const collection = client.db("UsersDB").collection("Users");
...
app.post("/expiry", function (req, res) {
  ...
  async function sendEmailtoReciever(Reciever, url){
	...
	client.connect((err) => {
	  ...
	  collection.findOne(query, function (err, result) {
		...
		if (result) {
			...
			if(result.Status != 'Completed' || result.Status != 'Void' || result.Status != 'Deleted'){
				const task = cron.schedule(' 0 0 0 */'+date+' * *',()=>{
